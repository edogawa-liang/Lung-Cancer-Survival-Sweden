...
- random sample(s): Yes! especially we ramdom pick 500 patient
- independent observations: maybe yes
- noninformative censoring
- right censored: satisfy
- Large sample: 500, satisfy
- proportional hazards: test below
*/
/* Censoring plot*/ 
...
/* 1) Fit Cox model WITHOUT Age (covariate of interest) */
proc phreg data=lungdata2 noprint;
   model treatment_days*survived(1) =
         /* continuous */
...
proc lifetest data=lungdata2 plots=survival;
    time treatment_days * survived(1);  /* 0=die, 1=survived(censored) */
    strata cancer_stage;    
    format cancer_stage cancer_stagefmt.;
run;
...
if _n_ <=77 then treatment="Radiation";
else if _n_ <=178 then treatment="Chemotherapy";
else if _n_ <=277 then treatment="Surgery";
else treatment="Combined";
run;
